        ...
        <div class="col-12">
            <main class="main">
                ...
                <section class="section section__form">
                    <form action="{{ route('bookings.store') }}" method="POST" class="form">
                        @csrf

                        {{-- Customer --}}
                        <div class="input-container">
                            <label for="user_id">Select customer</label>
                            <select name="user_id" id="user_id" class="input-box">
                                <option value="">--</option>
                        ...
                        <div class="input-container">
                            ...
                            <select name="igloo_id" id="igloo_id" class="input-box">
                                ...
                                <option value="{{ $igloo->id }}" {{ old('igloo_id') == $igloo->id ? 'selected' : '' }}>
                                    {{ $igloo->name }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-box d-flex justify-content-between row">
                            <div class="input-container col-6 ps-0">
                                ...
                                <input type="date" name="check_in_date" class="input-box" value="{{ old('check_in_date') }}" />
                                @error('check_in_date') <p style="color:#FF4D4D">{{ $message }}</p> @enderror
                            </div>

                            <div class="input-container col-6 pe-0">
                                <label for="check_out_date">Check out date</label>
                                <input type="date" name="check_out_date" class="input-box" value="{{ old('check_out_date') }}" />
                        ...
                        <div class="input-container">
                            ...
                            <textarea name="notes" id="notes" class="input-box">{{ old('notes') }}</textarea>
                        ...
                        <div class="form-box d-flex gap-5">
                            <div class="input-container input-container--checkbox">
                                ...
                                <input type="checkbox" name="early_check_in" id="early_check_in" value="1" {{ old('early_check_in') ? 'checked' : '' }} />
                            </div>

                            <div class="input-container input-container--checkbox">
                                <label for="late_check_out">Late check-out request</label>
                                <input type="checkbox" name="late_check_out" id="late_check_out" value="1" {{ old('late_check_out') ? 'checked' : '' }} />
                            </div>
                        </div>

                        <button class="button col-2 align-self-end">Add booking</button>
                    </form>
                </section>
            </main>
        </div>
